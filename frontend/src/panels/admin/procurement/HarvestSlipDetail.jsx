...
export default function HarvestSlipDetail() {
  ...
  const { harvestSlips, updateSlipStatus, convertSlipToTapal } = useAdminStore();
  const slip = harvestSlips.find(s => s.id === id);
  ...
  const handleConfirm = () => {
    const updates = slip.products.map(p => ({ ...p, confirmedQty: Number(partialQtys[p.id] ?? p.quantity) }));
    updateSlipStatus(slip.id, 'confirmed', { products: updates });
    toast.success('Confirmed with updated quantities');
  };
        ...
        <div className="lg:col-span-2 space-y-3">
          <Card className="border border-card-border shadow-subtle bg-white p-4">
            ...
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-black text-white flex items-center justify-center font-bold text-sm border border-black shrink-0 shadow-sm">{slip.farmer.name[0]}</div>
              <div className="space-y-0.5">
                <p className="text-sm font-bold text-black uppercase">{slip.farmer.name}</p>
                <p className="text-[9px] text-text-muted font-bold flex items-center gap-1.5"><Phone size={10} className="text-accent-olive" /> {slip.farmer.mobile}</p>
              </div>
            </div>
          </Card>
        ...
        <div className="space-y-3">
          ...
          <Card className="border border-card-border shadow-subtle bg-white p-4">
            ...
            <div className="space-y-2">
              {slip.status === 'pending' && <Button onClick={() => updateSlipStatus(slip.id, 'sent')} size="sm" className="w-full text-[9px] font-bold h-9 shadow-md bg-green-600 hover:bg-green-700">SEND WHATSAPP</Button>}
              {slip.status === 'sent' && <Button onClick={handleConfirm} size="sm" className="w-full text-[9px] font-bold h-9 shadow-md">CONFIRM QUANTITIES</Button>}
              {slip.status === 'confirmed' && <Button onClick={() => convertSlipToTapal(slip.id)} size="sm" className="w-full text-[9px] font-bold h-9 shadow-md bg-black text-white">GENERATE TAPAL</Button>}
              {['converted', 'rejected'].includes(slip.status) && <p className="text-[8px] text-text-muted font-bold text-center py-2 uppercase tracking-widest">RECORD LOCKED</p>}
            </div>
          </Card>
        </div>
      </div>
