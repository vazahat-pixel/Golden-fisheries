...
import { Plus, Search, AlertTriangle, ArrowLeft, Edit2, Trash2, X, Package } from 'lucide-react';
...
const RestaurantInventory = () => {
  ...
  const inventory = (kitchenStock || []).map((item) => ({
    id: item._id || item.id,
    name: item.name,
  ...
  const lowStockItems = inventory.filter((i) => i.stock < LOW_STOCK_KG);
        ...
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <p className="text-[9px] text-text-muted font-bold uppercase tracking-[0.3em] mt-1">
              Kitchen stock — Fish Mall accept se aata hai
            </p>
          </div>
        </div>
      ...
      <div className="space-y-6">
        <div className="flex flex-col lg:flex-row gap-4">
          ...
          <div className="bg-white border border-card-border p-4 px-6 flex items-center justify-between min-w-[240px] shadow-sm">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-0.5">LOW STOCK</p>
                <p
        ...
        {filteredInventory.length === 0 ? (
          <Card className="p-16 text-center border-dashed">
            ...
            <p className="text-sm font-black uppercase tracking-widest text-slate-400">No kitchen stock yet</p>
            <p className="text-[10px] text-slate-400 mt-2 max-w-md mx-auto">
              Fish Mall se internal bill bhejwayein, phir <strong>Received Stock</strong> par jaake Accept karein.
            </p>
            <Link to="/restaurant/received-stock" className="inline-block mt-6">
              <Button className="text-[10px] font-black uppercase">Open Received Stock</Button>
            </Link>
          </Card>
        ) : (
              ...
              >
                <div className="aspect-[16/9] bg-slate-50 flex items-center justify-center text-4xl relative group-hover:bg-slate-100 transition-colors">
                  ...
                  <div className="absolute top-3 left-3">
                    <Badge
                      className={`text-[7px] font-black border-none px-2 h-4 ${item.stock < LOW_STOCK_KG ? 'bg-red-600 text-white' : 'bg-black text-white'}`}
                    >
                      {item.stock < LOW_STOCK_KG ? 'CRITICAL' : 'NOMINAL'}
                    </Badge>
                  </div>
                </div>
                ...
                <div className="p-4 space-y-4">
                  ...
                  <div className="grid grid-cols-2 border border-card-border divide-x divide-card-border">
                    <div className="p-2.5 text-center bg-slate-50/50">
                      ...
                      <p className="text-sm font-serif italic font-black text-black">
