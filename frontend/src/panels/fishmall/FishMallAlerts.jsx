...
const FishMallAlerts = () => {
  ...
  useEffect(() => {
    markAlertsRead?.();
    fetchPendingTransfers?.();
  }, [markAlertsRead, fetchPendingTransfers]);
  ...
  const handleAcceptTransfer = async (alert) => {
    const transferId = alert.transferId;
    if (!transferId) {
      toast.error('Transfer ID missing — open Inventory page');
      return;
    }
    setAcceptingId(transferId);
    try {
      await acceptTransferAsync(transferId, {
        status: 'ACCEPTED',
        remarks: 'Accepted via alerts',
      });
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 border border-gray-100 rounded-2xl shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1">
              Procurement transfer — yahan accept karein
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Button
            ...
            variant="outline"
            className="text-[9px] font-black uppercase tracking-widest h-12 px-6 rounded-xl border-gray-200"
          >
            Scan System Now
          </Button>
        ...
        <div className="md:col-span-2 space-y-4">
          {alerts.length === 0 ? (
            ...
            alerts.map((alert) => {
              ...
              const canAccept = alert.type === 'PROCUREMENT_TRANSFER' && alert.canAccept && alert.transferId;
              return (
                <div
                  key={alert.id}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {alert.lines?.length > 0 && (
                      <ul className="mt-2 space-y-0.5">
                        {alert.lines.slice(0, 3).map((line, i) => (
                          <li key={i} className="text-[9px] font-bold text-gray-600">
                            {line.productName} — {line.quantity} {line.unit || 'KG'}
                          </li>
                        ))}
                      </ul>
                    )}
                    <div className="mt-4 flex items-center gap-2 flex-wrap">
                      {canAccept && (
                        <Button
                          size="sm"
                          className="h-8 text-[8px] font-black uppercase tracking-widest rounded-lg bg-emerald-600 hover:bg-emerald-700 border-none"
                          disabled={acceptingId === alert.transferId}
                          onClick={() => handleAcceptTransfer(alert)}
                        >
                          {acceptingId === alert.transferId ? 'Accepting…' : 'Accept Stock'}
                        </Button>
                      )}
                      <Button
                        ...
                        variant="ghost"
                        size="sm"
                        className="h-8 text-[8px] font-black uppercase tracking-widest rounded-lg text-gray-400 hover:text-red-500"
                      >
                        Dismiss
                      </Button>
                    </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-[#6B7550] p-8 rounded-3xl text-white space-y-4 relative overflow-hidden group">
            ...
            <p className="text-[9px] font-bold text-white/60 uppercase tracking-[0.2em]">
              Manual inflow band hai. Admin dispatch ke baad Accept dabayein.
            </p>
          </div>
        </div>
      </div>
