          ...
          {items.map((row) => (
            <li key={row.id}>
              <button
                ...
                className={`fa-surface w-full flex items-center gap-2.5 p-2.5 fa-tap transition-all duration-300 text-left ${
                  row.onClick ? 'hover:border-[var(--fa-accent)]/20' : ''
                }`}
              >
                <div className="w-9 h-9 rounded-full bg-[var(--fa-surface-elevated)] flex items-center justify-center shrink-0 overflow-hidden border border-[var(--fa-border)]">
                  {row.avatarUrl ? (
                ...
                <div className="flex-1 min-w-0 text-left">
                  ...
                  <p className="text-[10px] fa-muted mt-0.5">{row.subtitle}</p>
                </div>
